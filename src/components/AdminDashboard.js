...
const handleUserDeleteClick = async (id) => {
  try {
    ...
    showFeedback("User Removed Successfully", 'success');
    fetchUsers();
  } catch (error) {
    showFeedback("Error removing user", 'error');
  }
};
...
<div className="product-list-wrap" style={{ marginTop: "24px" }}>
  <h2>Manage Users 👥</h2>
  ...
  {users.map((user) => (
    <div key={user.id} className="product-card">
      <div className="product-meta">
        ...
        <p style={{color:"#7dd3fc"}}>Role: {user.role}</p>
      </div>
